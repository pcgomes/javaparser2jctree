 ...
 *
 * This file is part of JavaParser2JCTree.
 *
 * JavaParser2JCTree is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Lesser General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * JavaParser2JCTree is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOJCTree Object PARTICULAJCTree PURPOSE.  See the
 * GNU Lesser General Public License for more details.
 *
 * You should have received a copy of the GNU Lesser General Public License
 * along with JavaParser2JCTree.  If not, see <http://www.gnu.org/licenses/>.
 */
...
public class JavaParser2JCTree implements GenericVisitor<JCTree, Object> {
    ...
    private long setJCTreeModifiers(int ljpmodif, long lsource) {
        if (ModifierSet.isPublic(ljpmodif)) {
            lsource |= Flags.PUBLIC;
        }
        if (ModifierSet.isPrivate(ljpmodif)) {
            lsource |= Flags.PRIVATE;
        }
        if (ModifierSet.isProtected(ljpmodif)) {
            lsource |= Flags.PROTECTED;
        }
        if (ModifierSet.isStatic(ljpmodif)) {
            lsource |= Flags.STATIC;
        }
        if (ModifierSet.isFinal(ljpmodif)) {
            lsource |= Flags.FINAL;
        }
        if (ModifierSet.isSynchronized(ljpmodif)) {
            lsource |= Flags.SYNCHRONIZED;
        }
        if (ModifierSet.isVolatile(ljpmodif)) {
            lsource |= Flags.VOLATILE;
        }
        if (ModifierSet.isTransient(ljpmodif)) {
            lsource |= Flags.TRANSIENT;
        }
        if (ModifierSet.isStrictfp(ljpmodif)) {
            lsource |= Flags.STRICTFP;
        }
        if (ModifierSet.isNative(ljpmodif)) {
            lsource |= Flags.NATIVE;
        }
        if (ModifierSet.isAbstract(ljpmodif)) {
            lsource |= Flags.ABSTRACT;
        }
        ...
        if (n.getAnonymousClassBody() != null) {
            ...
            for (final BodyDeclaration member : n.getAnonymousClassBody()) {
                ...
                if (member instanceof TypeDeclaration) {
                    jcmodifier = setJCTreeModifiers(((TypeDeclaration) member).getModifiers(), jcmodifier);
                }
            }
    ...
    public JCTree visit(final UnaryExpr n, final Object arg) {
        ...
        switch (n.getOperator()) {
            ...
            case negative:
                ...
                break;
            case preIncrement:
                arg0 = JCTree.Tag.PREINC;
                break;
            case preDecrement:
                arg0 = JCTree.Tag.PREDEC;
                break;
            case not:
                arg0 = JCTree.Tag.NOT;
                break;
            case inverse:
                arg0 = JCTree.Tag.COMPL;
                break;
            case posIncrement:
                arg0 = JCTree.Tag.POSTINC;
                break;
            case posDecrement:
                arg0 = JCTree.Tag.POSTDEC;
                break;
            default:
                arg0 = JCTree.Tag.ERRONEOUS;
        }
